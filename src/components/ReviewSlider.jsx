...
export default function ReviewSlider({ reviews }) {
    ...
    const sents = splitSentences(rev.description);
    const hasMulti = sents.length > 1;

    return (
        <div style={{ position: 'relative' }}>
            {/* Card area */}
            <div style={{ position: 'relative', minHeight: cardHeight, display: 'flex', alignItems: 'center' }}>
                <AnimatePresence initial={false} custom={direction} mode="wait">
                    <motion.div
                        ...
                        style={{ width: '100%', cursor: 'grab', position: 'absolute' }}
                    >
                        <div ref={cardRef} style={{
                            ...
                            <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 16, marginTop: '2rem', paddingTop: '1.5rem', borderTop: '1px solid var(--border)' }}>
                                <div>
                                    <p style={{ fontWeight: 700, color: 'var(--text-primary)', fontSize: '0.95rem', marginBottom: 2 }}>{rev.name}</p>
                                    {rev.organization && (
                                        <p style={{ color: 'var(--text-muted)', fontSize: '0.82rem', fontFamily: 'var(--font-mono)', letterSpacing: '0.04em' }}>{rev.organization}</p>
                                    )}
                                </div>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                    <div style={{ display: 'flex', gap: 3 }}>
                                        {[...Array(5)].map((_, i) => (
                                            <Star key={i} size={14}
                                                style={{ color: i < rev.rating ? 'var(--primary)' : 'var(--border)', fill: i < rev.rating ? 'var(--primary)' : 'none' }}
                                            />
                                        ))}
                                    </div>
                                    <span className="pill pill-dim" style={{ fontSize: '0.65rem' }}>{rev.platform}</span>
                                </div>
                            </div>
                        </div>
                    </motion.div>
